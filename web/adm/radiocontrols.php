...
$(document).ready(function()
{
	_this = this;
  $.ajax({
    url: "/api/api.php",  data: "comp=" + comp + "&method=getclasses",
...
modifyControl = function (code, className, name, order, codes, cell) 
{
  ...
  let cellData = {row: position.row, col: position.col, name: name, order: order, codes: codes};
  
  var ret = $.ajax({
          url: "/api/api.php?method=deleteradiocontrol",
          data: "comp=" + comp + "&code=" + code + "&classname=" + className, 
          success: function() {loadControls(cellData);}
  });
}
  ...
  if (data != null && data.status == "OK" && data.splitcontrols != null) 
  {
    ...
    for (let i in classes) 
    {
      ...
      for (let j in codes) 
      {
        ...
        classControls.push({code: code, name: name, order: order});
      }
    ...
    {        
      ...
      for (let i in codes) 
      {
        ...
        columns.push({ "sTitle": title, "sClass": "right", "bSortable": true, "aTargets": [col++], "mDataProp": "controls",
          "render": function (data,type,row) {
            if (data[i].name)
            {
